package com.spring.boot.action.zk.server;

import com.google.gson.Gson;
import com.spring.boot.action.zk.constant.GlobalConstants;
...
public class DistributedSystemServer {
	...
	public static void main(String[] args) throws Exception {
		...
		server.connectZK("127.0.0.1", "2181");

		String path=GlobalConstants.parentZnodePath;

		List<String> children=server.getChildren(path,true);
		System.out.println("====>"+new Gson().toJson(children));
		for(String childPath:children){
			childPath=path+"/"+childPath;
		    List<String> childList=server.getChildren(childPath,true);
